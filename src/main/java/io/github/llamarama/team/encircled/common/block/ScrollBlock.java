package io.github.llamarama.team.encircled.common.block;

import io.github.llamarama.team.encircled.api.block.properties.ModBlockProperties;
import io.github.llamarama.team.encircled.common.block_entity.ScrollBlockEntity;
import io.github.llamarama.team.encircled.common.util.misc.Util;
...
public class ScrollBlock extends HorizontalFacingBlock implements BlockEntityProvider {
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        if (state.get(ModBlockProperties.OPEN)) {
            ...
            BlockPos left = pos.offset(direction, -1);
            BlockPos right = pos.offset(direction, 1);
            var downLeft = left.down();
            var downRight = right.down();

            boolean isLeftAir = world.getBlockState(left).isAir();
            boolean isRightAir = world.getBlockState(right).isAir();
            boolean downLeftFull = world.getBlockState(downLeft).isSideSolidFullSquare(world, downLeft, Direction.UP);
            boolean downRightFull = world.getBlockState(downRight).isSideSolidFullSquare(world, downRight, Direction.UP);
            boolean positionDown = world.getBlockState(pos.down()).isSideSolidFullSquare(world, pos, Direction.UP);

            return Util.allTrue(isLeftAir, isRightAir, downLeftFull, downRightFull, positionDown);
        } else {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return (beWorld, pos, cachedState, blockEntity) -> {
            if (!world.isClient && blockEntity instanceof ScrollBlockEntity) {
