package io.github.llamarama.team.encircled.client.render.guide;

import io.github.llamarama.team.encircled.client.EncircledClient;
import io.github.llamarama.team.encircled.common.network.ModNetworking;
import io.github.llamarama.team.encircled.common.network.packet.IncreaseChaosPacket;
import io.github.llamarama.team.encircled.common.network.packet.ReduceChaosPacket;
import io.github.llamarama.team.encircled.common.register.ModItems;
...
public class InitialGuideBookScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < ENTRIES.size(); i++) {
            ...
            EncircledClient.getGame().getItemRenderer().renderInGui(current.getDefaultStack(), 16 * i, 16);
        }
        int x = EncircledClient.getGame().getWindow().getWidth() / 2;
        int y = EncircledClient.getGame().getWindow().getHeight() / 2;

        EncircledClient.getGame().getItemRenderer().renderInGui(this.bookStack, x, y);
    ...
    protected void init() {
        ...
        this.increaseButton = this.addDrawableChild(new ButtonWidget(width / 2, height / 2, 100, 20,
            ...
            ClientPlayerEntity player = EncircledClient.getGame().player;
